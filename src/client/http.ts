...
import { AwNetworkError } from "./errors.js";
...
export const nodeHttpTransport: Transport = (request) =>
  new Promise<HttpResponse>((resolve, reject) => {
    ...
    if (url.protocol !== "http:" && url.protocol !== "https:") {
      reject(new AwNetworkError(`Unsupported protocol "${url.protocol}" in URL: ${request.url}`));
      return;
    }
      ...
      (res) => {
        ...
        res.on("data", (chunk: Buffer) => {
          ...
          if (maxBytes !== undefined && received > maxBytes) {
            aborted = true;
            res.destroy();
            reject(new AwNetworkError(`Response exceeded maxResponseBytes (${maxBytes})`));
            return;
          }
          chunks.push(chunk);
        });
        ...
        res.on("error", (err) => {
          ...
          reject(new AwNetworkError(`Response stream error: ${err.message}`, { cause: err }));
        });
      },
    );
    ...
    if (request.timeoutMs && request.timeoutMs > 0) {
      req.setTimeout(request.timeoutMs, () => {
        req.destroy(new AwNetworkError(`Request timed out after ${request.timeoutMs}ms`));
      });
    }

    req.on("error", (err) => {
      // A timeout destroy already passes an AwNetworkError; don't double-wrap.
      reject(err instanceof AwNetworkError ? err : new AwNetworkError(err.message, { cause: err }));
    });
