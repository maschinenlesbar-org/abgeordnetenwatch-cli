...
import { AwNetworkError, redactUrl } from "./errors.js";
...
export const nodeHttpTransport: Transport = (request) =>
  new Promise<HttpResponse>((resolve, reject) => {
    ...
    try {
      ...
      reject(new AwNetworkError(`Invalid URL: ${request.url}`));
      return;
    }
    ...
    if (url.protocol !== "http:" && url.protocol !== "https:") {
      reject(new AwNetworkError(`Unsupported protocol "${url.protocol}" in URL: ${redactUrl(request.url)}`));
      return;
    }
      ...
      (res) => {
        ...
        res.on("data", (chunk: Buffer) => {
          ...
          if (maxBytes !== undefined && received > maxBytes) {
            aborted = true;
            res.destroy();
            fail(new AwNetworkError(`Response exceeded maxResponseBytes (${maxBytes})`));
            return;
          }
          chunks.push(chunk);
        });
        ...
        res.on("error", (err) => {
          ...
          fail(new AwNetworkError(`Response stream error: ${err.message}`, { cause: err }));
        });
      },
    );
    ...
    if (request.timeoutMs && request.timeoutMs > 0) {
      ...
      timer = setTimeout(() => {
        const err = new AwNetworkError(`Request timed out after ${timeoutMs}ms`);
        fail(err);
        req.destroy(err);
    ...
    req.on("error", (err) => {
      // A timeout destroy already passes an AwNetworkError; don't double-wrap.
      fail(err instanceof AwNetworkError ? err : new AwNetworkError(err.message, { cause: err }));
    });
